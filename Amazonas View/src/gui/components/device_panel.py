...
class DevicePanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Hilos de conexiones anteriores que aun no han terminado. Hay que
        # conservarlos referenciados: si el recolector destruye un QThread
        # en marcha, Qt aborta el proceso entero.
        self._huerfanos:  list[DVRConnectWorker] = []
    ...
    def _on_connection_type_changed(self, conn_type: str):
        ...
        # Para credenciales, mostrar campos apropiados
    ...
    def _test_connection(self):
        ...
        # ── Relevo seguro del hilo anterior ──────────────────────────
        # OJO: esto cerraba la aplicacion entera. `quit()` solo termina el
        # bucle de eventos de un QThread; NO interrumpe un `run()` que
        # esta bloqueado haciendo I/O de red (que es justo lo que hace
        # DVRConnectWorker). Al reasignar `self._worker` se perdia la
        # unica referencia al hilo todavia vivo, el recolector lo
        # destruia y Qt abortaba el proceso con
        # "QThread: Destroyed while thread is still running".
        # Solucion: desconectar sus señales (para que no toque una UI que
        # ya no le corresponde) y CONSERVAR la referencia hasta que
        # termine solo.
        anterior = self._worker
        if anterior is not None:
            try:
                anterior.success.disconnect()
                anterior.error.disconnect()
    ...
    def _retirar_huerfano(self, worker) -> None:
        """Suelta un hilo antiguo cuando por fin termina."""
        try:
    ...
    def closeEvent(self, event):
        """Espera a los hilos antes de cerrar.

        Sin esto, cerrar la ventana con una conexion en curso destruye el
        QThread vivo y la aplicacion aborta al salir.
        """
        for worker in [self._worker, *self._huerfanos]:
        ...
        for ch in dev.channels:
            ...
            if dev.brand == "Hik-Connect":
                encoded = HikConnectChannelEncoder.encode_channel(
                    ...
                    rtsp_sub       = ch_dict.get("rtsp_sub", ""),
                    channel_id     = ch_dict.get("id", ""),
                    device_serial  = dev.username,  # App Key en serial temporal
                )
