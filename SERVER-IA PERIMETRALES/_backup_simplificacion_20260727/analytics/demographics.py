...
class DemographicsClassifier:
    ...
    def _try_body_sample(self, frame: np.ndarray, bbox: tuple,
                         acc: "_TrackAccumulator", track_id: int) -> bool:
        """Intenta una muestra CORPORAL (E3B) cuando no se vio cara.

        Aplica gates (tamano de persona, blur), corre el clasificador
        corporal y, si pasa, agrega la muestra al acumulador con peso 0.5 y
        source='body'. Devuelve True si agrego una muestra.
        """
        if not self._cfg.DEMO_BODY_BRANCH_ENABLED:
            return False
        if self._par is None and self._mivolo is None:
            return False
        x1, y1, x2, y2 = [int(v) for v in bbox]
        h, w = frame.shape[:2]
        x1, y1 = max(0, x1), max(0, y1)
        ...
        res = self._infer_body_only(crop)
        if res is None:
    ...
    def _infer_body_only(self, person_crop: np.ndarray):
        """Clasifica desde el cuerpo (sin cara). Primario PAR, fallback MiVOLO.
