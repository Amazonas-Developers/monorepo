...
class PersonAmazonas:
    ...
    def __init__(self,
        ...
        try:
            ...
            # Necesita el camera_id para cargar SU planograma, y ese solo se
            # conoce en process_frame -> inicializacion perezosa (ver
            # _ensure_retail). Sin planograma el modulo se apaga solo.
            self._retail: Optional[RetailAnalytics] = None
            self._retail_camera_id = None
    ...
    def _capture_detection(self, track_id, frame, box):
        ...
        try:
            ...
            try:
                ...
                pass
            # Copia ANOTADA inmediata para la galeria del cliente (se
            # re-anota con genero/edad cuando la demografia converge).
            self._write_client_capture(stem, meta0, person_img=person)
            # Foto del rostro (reusa el detector de la demografia)
            if hasattr(self, '_demographics') and self._demographics is not None:
                try:
                    face, _pose, _nw = \
                    ...
                    if face is not None and getattr(face, 'size', 0) > 0:
                        ...
                        cv2.imwrite(os.path.join(fdir, f"{stem}.jpg"), face,
    ...
    def draw_detections(self, image: np.ndarray, persons_inside: int) -> np.ndarray:
        ...
        if self.show_minimal_info:
            ...
            # Si no hay active_tracks usamos el contador heredado
            # persons_inside como fallback.
            total_unique = 0
            attended = 0
            active_persons = sum(
    ...
    def process_frame(self, image: np.ndarray, roi=None, activate_roi=False, camera_id: Any = 1, roi_escaparate=None, roi_prueba1=None, roi_prueba2=None, heatmap_activate: bool = False) -> Tuple[np.ndarray, Dict[str, Any]]:
        # Solo el modelo de personas es necesario (cosmeticos eliminado).
        ...
        with self._process_lock:
            ...
            try:
                ...
                # ── Analitica de supermercado ──
                # Pasillos, anaqueles vacios, toma/devolucion de producto,
                # carritos, decision entre productos y consulta de precios.
                # VA AQUI: despues de resolver persistent_id (todo se indexa
                # por persona, no por track) y ANTES de dibujar (necesita el
                # frame LIMPIO para medir el llenado de los anaqueles).
                # Sin condicionar a `.enabled`: la deteccion de CAJAS corre
                # SIEMPRE (con o sin planograma); update() decide por dentro
                # que partes del pipeline ejecutar.
                _retail = self._ensure_retail(camera_id)
                if _retail is not None:
                    _retail.update(inference_image, self.active_tracks,
                                   self.frame_counter)
                ...
                if _draw_server:
                    ...
                    # Zonas del supermercado encima: pasillos con su aforo,
                    # anaqueles coloreados por nivel de stock, maquina de
                    # precios, carritos y cajas en el piso (estas ultimas se
                    # pintan SIEMPRE, con o sin planograma).
                    if _retail is not None:
                        processed_image = _retail.draw(processed_image)
                else:
                ...
                # Analitica de supermercado: resumen en cada frame y reporte
                # completo de marketing (cacheado/throttled + persistido).
                if _retail is not None:
                    metadata['retail'] = _retail.get_stats()
                    if _retail.enabled:
                        metadata['retail_report'] = _retail.get_report()
                        # Persiste a JSON solo cuando el reporte se regenero
                        # (no en cada frame).
                        _retail.maybe_save_report()
    ...
    def reset_counter(self):
        ...
        # Analitica de supermercado (mantiene el planograma, borra metricas).
        if getattr(self, '_retail', None) is not None:
            self._retail.reset()
    ...
    def _run_analytics(self, frame: np.ndarray):
        ...
        try:
            ...
            # 5. Log periodico del reporte de calidad facial (Hito 3): deja
    ...
    # ── Analitica de supermercado (pasillos / anaqueles / compra) ───

    def _ensure_retail(self, camera_id: Any):
        """Crea (una vez) el orquestador de analitica retail de esta camara.

        Es perezoso porque el planograma se carga por camera_id y ese dato
        solo llega en process_frame. Si falla, se desactiva y el resto del
        pipeline sigue intacto.
        """
        if self._retail is not None and self._retail_camera_id == camera_id:
            return self._retail
        try:
            self._retail = RetailAnalytics(
                camera_id=camera_id,
                device=self.device,
                demographics=getattr(self, '_demographics', None),
                analytics_logger=getattr(self, '_analytics_logger', None),
                # Personal por FOTO: reusa la galeria biometrica viva para
                # verificar quien es empleado (config/personal/*.jpg).
                reidentifier=getattr(self, '_reidentifier', None),
                staff_yunet_path=_YUNET_MODEL,
            )
            self._retail_camera_id = camera_id
            if self._retail.enabled:
                lay = self._retail.layout
                print(f"🛒 Analitica de supermercado ACTIVA (cam {camera_id}): "
                      f"{len(lay.aisles)} pasillos, {len(lay.shelves)} "
                      f"anaqueles, {len(lay.fixtures)} mobiliario")
        except Exception as e:  # noqa: BLE001
            logger.error("No se pudo iniciar la analitica retail: %s", e,
                         exc_info=True)
            self._retail = None
            self._retail_camera_id = camera_id
        return self._retail
    ...
    def define_aisle(self, nombre: str, poligono, categoria: str = "") -> bool:
        """Define un PASILLO (poligono) y lo persiste en el planograma.

        Las coordenadas pueden ser px o normalizadas 0..1 (recomendado).
        """
        r = self._ensure_retail(self._retail_camera_id)
        return bool(r and r.define_aisle(nombre, poligono, categoria))

    def define_shelf(self, nombre: str, rect, pasillo: str = "",
                     categoria: str = "", precio: float = 0.0,
                     sku: str = "") -> bool:
        """Define un ANAQUEL. Su `nombre` es el producto que se reporta."""
        r = self._ensure_retail(self._retail_camera_id)
        return bool(r and r.define_shelf(nombre, rect, pasillo, categoria,
                                         precio, sku))

    def define_price_checker(self, nombre: str, rect) -> bool:
        """Define la zona de la MAQUINA CONSULTORA DE PRECIOS."""
        r = self._ensure_retail(self._retail_camera_id)
        return bool(r and r.define_price_checker(nombre, rect))

    def capture_shelf_references(self, frame: np.ndarray = None) -> int:
        """Calibra el 100% de llenado de todos los anaqueles.

        Se llama con los estantes repuestos y SIN gente delante.
        """
        r = self._ensure_retail(self._retail_camera_id)
        if r is None:
            return 0
        f = frame if frame is not None else self.last_processed_frame
        return r.capture_shelf_references(f) if f is not None else 0

    def get_retail_report(self) -> Dict[str, Any]:
        """Reporte completo de marketing/ventas de esta camara."""
        r = self._ensure_retail(self._retail_camera_id)
        return r.get_report(force=True) if r else {'activo': False}
    ...
    def get_analytics_stats(self) -> Dict[str, Any]:
        ...
        if hasattr(self, '_stock_monitor'):
            stats['stock'] = self._stock_monitor.get_stats()
        if getattr(self, '_retail', None) is not None:
            stats['retail'] = self._retail.get_stats()
        return stats
