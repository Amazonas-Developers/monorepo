...
class Render_box(QFrame):
    ...
    def setup_ui(self):
        ...
        # ── Barra SIMPLIFICADA: el toggle de IA + 3 menus desplegables que
        # agrupan todas las opciones (antes ~16 botones sueltos). ──
        self._menu_roi = self._make_menu_button(
            "ROI ▾",
            "Zonas de interes: ROI principal, toma de orden, entrega, reiniciar",
        ...
        self._menu_ia = self._make_menu_button(
            "Asistente ▾",
            "Asistente IA: preguntar, detectar evento, modelo (3B/7B), contexto",
            self._build_ia_menu)

        # Layout: [IA | ROI | Vista | Asistente | Clases] --- [Captura ▶ ⏸ ⏹ DVR]
    ...
    def _build_roi_menu(self, menu):
        ...
        self._add_toggle(menu, "ROI Toma de orden (azul)",
                         self.order_zone_boolean, self._toggle_order_zone)
        self._add_toggle(menu, "ROI Entrega de plato (rojo)",
                         self.delivery_zone_boolean, self._toggle_delivery_zone)
        menu.addSeparator()
    ...
    def loop_show_result(self):
        ...
        try:
            ...
            # invocaciones. Drenamos TODOS los mensajes completos y nos
            ...
            if now - self.last_fps_time >= 1.0:
                ...
                # (frames procesados por el servidor). Aqui solo mostramos la
            ...
            if self.socket is not None:
                ...
                # mostramos el frame crudo local. (Antes habia aqui una llamada
                # recursiva a loop_show_result: se elimino, causaba reentrancia
                # y lecturas en vacio.)
                if not self.smart_mode:
