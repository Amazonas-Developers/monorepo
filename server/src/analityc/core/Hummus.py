...
# ── Rutas por defecto ─────────────────────────────────────────────────────────
# Raiz del servidor: `server/src/analityc/core/Hummus.py` -> parents[3].
# Antes eran rutas ABSOLUTAS a esta maquina (y con las barras dobladas dentro
# de una cadena CRUDA, o sea `C:\\Users\\...`, que colaba solo porque Windows
# colapsa separadores repetidos). Al mover el servidor a `server/` el
# 30-jul-2026 dejaron de existir. Ancladas a `__file__` (regla 6).
...
DEFAULT_PERSON_MODEL_PATH       = str(_MODELOS / "yolo11l.pt")
DEFAULT_ORDER_SCREENSHOT_DIR    = str(_SALIDA / "Toma_de_orden_hummus")
DEFAULT_DELIVERY_SCREENSHOT_DIR = str(_SALIDA / "Entrega_de_plato_hummus")
DEFAULT_ALERT_CSV_PATH          = str(_SALIDA / "hummus_alertas.csv")
DEFAULT_ALERT_PENDING_CSV_PATH  = str(_SALIDA / "hummus_alertas_pending.csv")
...
class Recepcion_Marcador(HummusProcess):
    ...
    _DEFAULT_SCREENSHOT_DIR = str(_SALIDA / "Recepcion_Marcador")
...
class Entrega_Marcador(HummusProcess):
    ...
    _DEFAULT_SCREENSHOT_DIR = str(_SALIDA / "Entrega_Marcador")
