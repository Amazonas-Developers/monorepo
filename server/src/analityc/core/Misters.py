...
# Raiz del servidor: `server/src/analityc/core/Misters.py` -> parents[3].
# Antes estos nueve valores eran rutas ABSOLUTAS a esta maquina (y ademas con
# las barras dobladas dentro de una cadena CRUDA, asi que quedaban como
# `C:\\Users\\...`: funcionaba solo porque Windows colapsa los separadores
# repetidos). Al mover el servidor a `server/` el 30-jul-2026 dejaron de
# existir. Se anclan a `__file__`, que es lo que pide la regla 6.
_RAIZ_SERVIDOR = Path(__file__).resolve().parents[3]
_MODELOS = _RAIZ_SERVIDOR / "models" / "base"
_SALIDA = _RAIZ_SERVIDOR / "output"

DEFAULT_MODEL_PATH = str(_MODELOS / "1080.pt")
DEFAULT_PERSON_MODEL_PATH = str(_MODELOS / "yolo12m.pt")
DEFAULT_OCCUPY_SCREENSHOT_DIR = str(_SALIDA / "Cliente_ocupa_mesa_misters")
DEFAULT_VACATE_SCREENSHOT_DIR = str(_SALIDA / "Cliente_desocupa_mesa_misters")
DEFAULT_MANAGER_SCREENSHOT_DIR = str(_SALIDA / "Toque_de_gerente_misters")
DEFAULT_ASSISTANT_SCREENSHOT_DIR = str(_SALIDA / "Toque_de_asistente_misters")
DEFAULT_ALERT_CSV_PATH = str(_SALIDA / "misters_alertas.csv")
DEFAULT_ALERT_EXCEL_PATH = str(_SALIDA / "misters_alertas.xlsx")
DEFAULT_ALERT_PENDING_CSV_PATH = str(_SALIDA / "misters_alertas_pending.csv")
