...
class FaceReidentifier:
    ...
    def __init__(self,
        ...
        # Namespace numerico por camara: como el reidentificador es COMPARTIDO
        # entre camaras y los track_id son locales a cada camara (ByteTrack
        # empieza en 1 en cada una), sin esto los track_id colisionarian en
        # _track_to_person (camara A track 1 == camara B track 1). Asigna un
        # offset unico y estable por camara. Ver camera_track_namespace().
        self._cam_ns: Dict[str, int] = {}
    ...
    def _best_in_gallery(self, emb: np.ndarray
                         ) -> Tuple[Optional[str], float]:
        """Mejor match de un embedding contra TODA la galeria.

        Para cada persona toma la maxima similitud coseno entre el query y
        CUALQUIERA de sus plantillas (multi-embedding). NO aplica umbral ni
        toma el lock (el llamador debe tenerlo). Reutilizado por
        identify_or_register y search_gallery (una sola fuente de verdad).

        Args:
            emb: embedding facial (512,) L2-normalizado.

        Returns:
            (persistent_id, mejor_similitud). (None, -1.0) si la galeria
            esta vacia.
        """
    ...
    def search_gallery(self, embedding: np.ndarray,
                       threshold: Optional[float] = None,
                       ) -> Optional[Tuple[str, float]]:
        """Busca el mejor match en la galeria (solo LECTURA, no registra).

        Util como bloque base del Re-ID, para diagnostico y para consultas.
        No modifica la galeria ni el mapeo track->persona.

        Args:
            embedding: vector facial (512,) L2-normalizado, de
                compute_embedding(). Si es None devuelve None.
            threshold: cosine minimo para considerar match. Si None usa
                self._sim_threshold (config REID_SIMILARITY_THRESHOLD).

        Returns:
            (persistent_id, score) del mejor match SI score >= threshold;
            None si no hay match (galeria vacia o por debajo del umbral).
        """
        if embedding is None or self._session is None:
            return None
        thr = self._sim_threshold if threshold is None else float(threshold)
        with self._lock:
            best_uid, best_sim = self._best_in_gallery(embedding)
        if best_uid is not None and best_sim >= thr:
    ...
    def identify_or_register(self, track_id: int,
        ...
        with self._lock:
            ...
            # Buscar mejor match: para cada persona, la maxima similitud
            # entre el query y CUALQUIERA de sus embeddings (multi-embedding).
            # Misma logica que search_gallery -> _best_in_gallery (no duplicar).
            best_uid, best_sim = self._best_in_gallery(emb)
            ...
            now = time.time()
            # Sembrar los votos (Hito 6) con la primera lectura si es confiable.
            _minv = float(AnalyticsConfig.DEMO_VOTE_MIN_CONFIDENCE)
            _seed_votos = {}
            _seed_edades = []
            if (float(demo_confidence) >= _minv
                    and gender in ('Hombres', 'Mujeres', 'Ninos')):
                _seed_votos[gender] = float(demo_confidence)
            if float(demo_confidence) >= _minv and age_value and float(age_value) > 0:
                _seed_edades = [float(age_value)]
            new_rec = {
                ...
                'votos_genero': _seed_votos,
                'estimaciones_edad': _seed_edades,
                'first_seen': now,
                'last_seen': now,
                'visit_count': 1,
    ...
    def _maybe_update_demographics(self, rec: Dict[str, Any],
        ...
        """Acumula la demografia de la persona por VOTOS y la consolida (Hito 6).

        Cada lectura ya viene suavizada por track (voting temporal + trimmed
        mean en demographics.py); aqui se agrega A NIVEL DE PERSONA para que
        sea estable entre tracks/visitas:
         - genero: voto PONDERADO por confianza (`votos_genero[g] += conf`) ->
           se reporta el MAYORITARIO.
         - edad: lista de estimaciones (`estimaciones_edad`) -> se reporta la
           MEDIANA (robusta a outliers), mapeada a rango via AGE_RANGES.
        Reglas:
         - `manual_override` = True -> NUNCA tocar (correccion del usuario).
         - Solo cuentan lecturas con confianza >= DEMO_VOTE_MIN_CONFIDENCE.
         - `demo_confidence` se mantiene como el MAXIMO visto (compat con los
           gates existentes que lo consultan).
        """
        if rec.get('manual_override'):
            return  # Correccion manual del usuario es sticky
        if gender is None and age_range is None:
            return
        conf = float(demo_confidence)
        min_vote = float(AnalyticsConfig.DEMO_VOTE_MIN_CONFIDENCE)
        # demo_confidence = maximo historico (no romper gates existentes).
        rec['demo_confidence'] = max(float(rec.get('demo_confidence', 0.0)), conf)

        # Acumular votos solo si la lectura es suficientemente confiable.
        if conf >= min_vote:
            if gender in ('Hombres', 'Mujeres', 'Ninos'):
                votos = rec.setdefault('votos_genero', {})
                votos[gender] = float(votos.get(gender, 0.0)) + conf
            if age_value and float(age_value) > 0:
                est = rec.setdefault('estimaciones_edad', [])
                est.append(float(age_value))
                max_est = int(AnalyticsConfig.DEMO_AGE_MAX_ESTIMATES)
                if len(est) > max_est:
                    del est[0:len(est) - max_est]

        # Consolidar GENERO desde los votos (mayoritario por confianza).
        votos = rec.get('votos_genero') or {}
        if votos:
            rec['gender'] = max(votos.items(), key=lambda kv: kv[1])[0]
        elif gender is not None and rec.get('gender') in (None, "Desconocido"):
            rec['gender'] = gender  # sin votos aun: usar la lectura cruda

        # Consolidar EDAD desde la mediana de estimaciones.
        est = rec.get('estimaciones_edad') or []
        if est:
            med = float(np.median(np.asarray(est, dtype=np.float64)))
            rec['age_value'] = round(med, 1)
            rec['age_range'] = AnalyticsConfig.age_range_from_value(int(med))
        elif age_range is not None and rec.get('age_range') in (None,
                                                               "Desconocido"):
    ...
    def camera_track_namespace(self, camera_id: Any) -> int:
        """Offset numerico UNICO y estable por camara para los track_id.

        Evita que los track_id locales de distintas camaras colisionen en el
        mapeo compartido `_track_to_person` (cada ByteTrack numera desde 1).
        La 1a camara recibe 100000 (compat 1-camara con el offset historico);
        las siguientes 100000 + N*100_000_000 (separacion >> ids de ByteTrack,
        y muy por debajo de 2^53 para serializar a JSON sin perder precision).

        Args:
            camera_id: identificador de la camara (puede ser UUID/str/int).

        Returns:
            offset entero a sumar al track_id local de esa camara.
        """
        key = str(camera_id)
        with self._lock:
            off = self._cam_ns.get(key)
            if off is None:
                off = 100000 + len(self._cam_ns) * 100_000_000
                self._cam_ns[key] = off
            return off

    def get_persistent_id(self, track_id: int) -> Optional[str]:
        """Devuelve el persistent_id (person_uuid) mapeado a un track_id, o
        None si ese track aun no se ha confirmado con rostro.

        Es la consulta ligera (O(1), sin construir demograficos) que usa la
        capa de resolucion de identidad del Hito 4 para saber si un track ya
        tiene identidad de largo plazo confirmada.

        Args:
            track_id: id de tracking (la MISMA clave usada en
                identify_or_register, = clave de active_tracks).

        Returns:
            person_uuid (str) si el track esta mapeado; None si no.
        """
