...
def process_image_sync(
    ...
    entrega_roi: Any = None,
    entrega_roi_activate: bool = False,
    prueba1_roi: Any = None,
    prueba1_roi_activate: bool = False,
    prueba2_roi: Any = None,
    prueba2_roi_activate: bool = False,
    order_zone: Any = None,
    ...
    if isinstance(processor, PersonAmazonas):
        esc_roi = entrega_roi if (entrega_roi_activate and isinstance(entrega_roi, list) and len(entrega_roi) >= 3) else None
        p1_roi = prueba1_roi if (prueba1_roi_activate and isinstance(prueba1_roi, list) and len(prueba1_roi) >= 3) else None
        p2_roi = prueba2_roi if (prueba2_roi_activate and isinstance(prueba2_roi, list) and len(prueba2_roi) >= 3) else None
        try:
            ...
            if camera_name:
                cam_proc._camera_display_name = camera_name
            cam_proc._draw_server = bool(draw_server)
            return cam_proc.process_frame(img, roi, roi_activate, camera_id, roi_escaparate=esc_roi, roi_prueba1=p1_roi, roi_prueba2=p2_roi, heatmap_activate=heatmap_activate)
        except Exception:
            if camera_angle:
                processor.set_camera_angle(camera_angle)
            if camera_name:
                processor._camera_display_name = camera_name
            processor._draw_server = bool(draw_server)
            return processor.process_frame(img, roi, roi_activate, camera_id, roi_escaparate=esc_roi, roi_prueba1=p1_roi, roi_prueba2=p2_roi, heatmap_activate=heatmap_activate)
...
def _full_frame_sync(
    ...
    processed_img, metadata = process_image_sync(
        ...
        entrega_roi=entrega_roi, entrega_roi_activate=entrega_roi_activate,
        prueba1_roi=prueba1_roi, prueba1_roi_activate=prueba1_roi_activate,
        prueba2_roi=prueba2_roi, prueba2_roi_activate=prueba2_roi_activate,
...
# ──────────────────────────────────────────────────────────────────────────
#  Analitica de supermercado (retail)
#    GET  /retail/cameras          camaras con analitica activa
#    GET  /retail/report           reporte de marketing/ventas de una camara
#    GET  /retail/layout           planograma (pasillos/anaqueles/mobiliario)
#    POST /retail/calibrate        fija el 100% de llenado de los anaqueles
#  El cliente (windows_managers_view) los consume desde el menu "Tienda".
# ──────────────────────────────────────────────────────────────────────────
def _iter_person_processors():
    ...
    for cid, info in list(active_connections.items()):
        ...
        if cams:
            ...
            # Instancia unica (sin multi-camara): usa su propio camera_id.
            yield cid, getattr(proc, "_retail_camera_id", None), proc


def _find_camera_processor(camera_id: Optional[str]):
    """Procesador de esa camara. Sin camera_id devuelve el primero que tenga
    analitica retail activa (caso tipico de una sola camara)."""
    primero = None
    for _cid, cam_id, cam_proc in _iter_person_processors():
        if camera_id and str(cam_id) == str(camera_id):
            return cam_proc
        if primero is None:
            retail = getattr(cam_proc, "_retail", None)
            if retail is not None and retail.enabled:
                primero = cam_proc
    return None if camera_id else primero


@app.get("/retail/cameras")
def retail_cameras():
    """Lista las camaras y si tienen planograma/analitica de tienda activa."""
    try:
        out = []
        for cid, cam_id, cam_proc in _iter_person_processors():
            retail = getattr(cam_proc, "_retail", None)
            out.append({
                "client_id": cid,
                "camera_id": str(cam_id),
                "retail_activo": bool(retail is not None and retail.enabled),
                "planograma": (retail.layout.path if retail is not None
                               else None),
            })
        return {"status": "ok", "camaras": out}
    except Exception as exc:
        logger.exception("retail_cameras error")
        return {"status": "error", "message": str(exc)}


@app.get("/retail/report")
def retail_report(camera_id: str = ""):
    """Reporte completo de marketing/ventas de una camara."""
    try:
        cam_proc = _find_camera_processor(camera_id or None)
        if cam_proc is None:
            return {"status": "error",
                    "message": f"sin analitica retail para camara "
                               f"'{camera_id or '(cualquiera)'}'"}
        return {"status": "ok", "report": cam_proc.get_retail_report()}
    except Exception as exc:
        logger.exception("retail_report error")
        return {"status": "error", "message": str(exc)}


@app.get("/retail/layout")
def retail_layout(camera_id: str = ""):
    """Planograma vigente de la camara (pasillos, anaqueles, mobiliario)."""
    try:
        cam_proc = _find_camera_processor(camera_id or None)
        retail = getattr(cam_proc, "_retail", None) if cam_proc else None
        if retail is None:
            return {"status": "error", "message": "sin planograma cargado"}
        return {"status": "ok", "layout": retail.layout.to_dict(),
                "ruta": retail.layout.path}
    except Exception as exc:
        logger.exception("retail_layout error")
        return {"status": "error", "message": str(exc)}


@app.post("/retail/layout")
def retail_set_layout(camera_id: str = Form(...), layout: str = Form(...)):
    """Guarda el planograma que dibujo el CLIENTE sobre su propio frame.

    En esta arquitectura el servidor nunca ve la camara (el cliente captura
    y le envia los frames), asi que las zonas se dibujan en el cliente y se
    suben aqui. `layout` es el JSON con pasillos/anaqueles/mobiliario en
        ...
        if cam_proc is None:
            return {"status": "error",
                    "message": f"camara '{camera_id}' no encontrada; "
                               f"enciende el analisis IA de esa camara antes "
                               f"de guardar el planograma"}
        retail = cam_proc._ensure_retail(camera_id)
        if retail is None:
            return {"status": "error",
                    "message": "no se pudo iniciar la analitica retail"}
        retail.layout.from_dict(data)
        if not retail.layout.save():
            return {"status": "error", "message": "no se pudo escribir el "
                                                  "planograma en disco"}
        return {
            "status": "ok",
            "ruta": retail.layout.path,
            "pasillos": len(retail.layout.aisles),
            "anaqueles": len(retail.layout.shelves),
            "mobiliario": len(retail.layout.fixtures),
        }
    except Exception as exc:
        logger.exception("retail_set_layout error")
        return {"status": "error", "message": str(exc)}


@app.post("/retail/calibrate")
def retail_calibrate(camera_id: str = Form("")):
    """Fija la referencia de "estante lleno" de todos los anaqueles.

    Usa el ULTIMO frame procesado de esa camara, asi que hay que llamarlo con
    los estantes repuestos y sin clientes delante.
    """
    try:
        cam_proc = _find_camera_processor(camera_id or None)
        if cam_proc is None:
            return {"status": "error",
                    "message": "sin analitica retail para esa camara"}
        n = cam_proc.capture_shelf_references()
        if n == 0:
            return {"status": "error",
                    "message": "no se pudo calibrar (sin frame reciente o "
                               "sin anaqueles en el planograma)"}
        return {"status": "ok", "anaqueles_calibrados": n}
    except Exception as exc:
        logger.exception("retail_calibrate error")
        return {"status": "error", "message": str(exc)}