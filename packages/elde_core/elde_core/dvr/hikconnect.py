...
class HikConnectStrategy(DVRStrategy):
    ...
    def _get_live_url(self, resource_id: str, serial: str,
        ...
        """
        # Se anota si el HLS fue rechazado por traer segmentos ErrCode: es la
        # señal de que ESE equipo está bloqueado para stream por la nube
        # (medido: cuando pasa, el RTMP tampoco entrega imagen). Lo consulta
        # get_device_info para no marcar el canal como reproducible.
        self._hls_bloqueado = False
        for protocol in [2, 1, 3]:  # HLS → RTSP → RTMP
            try:
                ...
                # OJO: la URL trae query (...m3u8?expire=...&id=...), así que
                # hay que buscar ".m3u8" DENTRO, no con endswith (si no, la
                # verificación nunca corría y se devolvían URLs con ErrCode).
                if "ezvizlife.com" in url and ".m3u8" in url:
                    proto_name = "HLS"
                    self._log(f"   🔍 EZVIZ HLS detectado, verificando segmentos...")
                    if not self._check_hls_segments(url):
                        self._hls_bloqueado = True
    ...
    def get_device_info(self) -> DeviceInfo:
        ...
        for i, cam in enumerate(cameras, 1):
            ...
            # NO filtrar por el campo `online` de la cámara: es POCO FIABLE.
            # Medido contra la cuenta real: el DVR PROLICOR reporta online="0"
            # en todos sus canales y sin embargo transmiten a 1280x720, mientras
            # que otro equipo reporta online="1" y devuelve ErrCode. Se pide la
            # URL siempre y manda el resultado real; `online` solo se usa como
            # dato informativo del estado.
            # El código de verificación (si el usuario lo dio) desbloquea los
            # streams cifrados: la nube lo exige para devolver una URL usable.
            codigo = getattr(self, "verification_code", "") or ""
            rtsp_main = self._get_live_url(rid, dev_ser, quality=1, code=codigo)
            bloqueado_nube = bool(getattr(self, "_hls_bloqueado", False))
            ...
            if rtsp_main == "ENCRYPTED":
                ...
                # El sub-stream se reutiliza en vez de pedirlo aparte: cada
                # petición de URL puede probar 3 protocolos + descargar el
                # m3u8 para validarlo, así que pedir main+sub en 20 canales
                # llegaba a varios minutos de espera. El cliente reproduce
                # rtsp_main; si algún día hace falta la sub-calidad real, se
                # pide bajo demanda con refresh_live_url().
                rtsp_sub = rtsp_main
                if rtsp_main and not bloqueado_nube:
                    self._log(f"   🎬 {ch_name}: {rtsp_main[:90]}...")
                elif rtsp_main and bloqueado_nube:
                    self._log(f"   ⛔ {ch_name}: la nube devuelve ErrCode para "
                              f"este equipo; el stream NO se reproducirá "
                              f"(se guarda la URL alternativa por si acaso)")
                else:
                    self._log(f"   ❌ {ch_name}: SIN URL (el equipo no entrega "
                              f"stream por la nube)")

            info.channels.append(ChannelInfo(
                id=rid or str(i), name=ch_name,
                # Estado REAL: hay stream y la nube no lo tiene bloqueado.
                status=("active" if (rtsp_main and not bloqueado_nube)
                        else "inactive"),
                ...
                extra={
                    ...
                    "is_hikconnect": True,
                    # True = la nube respondió ErrCode para este equipo; la URL
                    # existe pero no dará imagen (útil para avisar en la UI).
                    "bloqueado_nube": bloqueado_nube,
                },
            ))
    ...
    def refresh_live_url(
        ...
        for protocol in [2, 1, 3]:
            try:
                ...
                # MISMO arreglo que en _get_live_url: la URL trae query
                # (...m3u8?expire=...&id=...), asi que endswith() no acierta
                # nunca y la verificacion no se ejecutaba. Aqui se habia quedado
                # sin corregir; lo detecto la prueba del nucleo al reconciliar.
                if "ezvizlife.com" in url and ".m3u8" in url:
                    try:
