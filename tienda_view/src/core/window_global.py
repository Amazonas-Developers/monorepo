...
class Windows_monitor(QObject):
    ...
    def __init__(self, parent=None, main_window=None):
        if not hasattr(self, '_initialized'):
            ...
            # OJO: este singleton se construye al importar el modulo, ANTES de
            # que exista la QApplication, asi que aqui todavia no se puede
            # enganchar aboutToQuit; lo hace main.py con stop_scanner(). Esto
            # queda solo como ultima red: destroyed llega tarde (el hilo ya
            # se esta destruyendo) pero no estorba.
            self.destroyed.connect(self._scanner_thread.stop)

    def stop_scanner(self):
        """Detiene el hilo de escaneo de ventanas.

        Idempotente: se puede llamar varias veces (aboutToQuit y ademas
        destroyed) sin efecto secundario."""
        ...
        try:
            ...
            pass                 # el objeto C++ ya lo destruyo Qt
